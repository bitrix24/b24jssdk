...
description: 'The primary method for executing Bitrix24 REST API calls via webhook. Allows calling any REST API method with specified parameters. Works only in a server environment. Works only in server environment.'
navigation:
  title: callMethod
links:
  - label: B24Hook
...
::note{title="Automatically retrieves all items" to="/docs/hook/methods/call-fast-list-method/"}
