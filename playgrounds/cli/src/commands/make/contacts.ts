...
import { LANGUAGES, EMAIL_DOMAINS, SOURCES, POSTS, CLIENT_ERROR_STATUS } from '../../constants'
...
export default defineCommand({
  ...
  async setup({ args }) {
    ...
    const { b24, logger } = createB24Client('contacts')
    ...
    async function createRandomContacts(): Promise<void> {
      logger.notice(`${icon('🚀 ')}Starting creation of random contacts in Bitrix24`)
      logger.notice(`${icon('📊 ')}Planned to create: ${params.total} contacts`)
      ...
      logger.notice(`${icon('📈 ')}Successfully created: ${createdCount} contacts`)
      logger.notice(`${icon('⏱️ ')}Total execution time: ${duration} seconds`)
      logger.notice(`${icon('📊 ')}Average time per contact: ${(Number(duration) / params.total).toFixed(2)} seconds`)
