...
export function useTheme() {
  ...
  function exportConfig(): string {
    track('Theme Exported', { type: 'Config', framework: restApiVersion.value })

    const configString = JSON.stringify('', null, 2)
      .replace(/"([^"]+)":/g, '$1:')
      .replace(/"/g, '\'')
  ...
  return {
    ...
    hasCSSChanges,
    hasConfigChanges,
    configLabel: computed(() => restApiVersion.value === 'vue' ? 'vite.config.ts' : 'app.config.ts'),
    exportCSS,
    exportConfig,
    applyThemeSettings,
    resetTheme
