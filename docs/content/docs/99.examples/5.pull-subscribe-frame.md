...
- `initB24Helper($b24, dataTypes, requestId)` loads the data the Pull client needs to register a channel. `usePullClient()` (called without an explicit `userId`) reads `profileInfo.data.id`, so at minimum `LoadDataType.Profile` must be loaded first — otherwise the call throws `B24HelperManager.profileInfo not initialized`. Loading `LoadDataType.App` as well (as the example does) is recommended so the manager has the full app context.
