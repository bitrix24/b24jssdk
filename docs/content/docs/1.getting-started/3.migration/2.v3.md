...
- `getTotal()` / `isMore()` / `hasMore()` — revisited when `actions.v3.aggregate` is verified against a live portal and loses `@experimental` (a v3 count would then exist, giving `getTotal()` a replacement for the first time), **or** when a `restApi:v2` sunset date is announced.
