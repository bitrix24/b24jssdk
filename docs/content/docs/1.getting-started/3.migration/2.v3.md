...
That makes reading it a compile error until you narrow — which is the point. The
report behind this change was a condition passed to a slider and read back as
`options.payload.id`{lang="ts-type"}: it compiled without a word, and was
`undefined`{lang="ts-type"} at run time.

```diff
- const place = $b24.placement.options?.place
+ const raw = $b24.placement.options['place']
+ const place = 'string' === typeof raw ? raw : undefined
```
...
- **`options` is never `undefined`.** The portal sends an object, a JSON string,
  ...
  used to need is now dead weight.
...
`isSliderMode`{lang="ts-type"} is unchanged, and still reads `IFRAME === 'Y'`
— a string, because on the default-placement path it came from that query string.
