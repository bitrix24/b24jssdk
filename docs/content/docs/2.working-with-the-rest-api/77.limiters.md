...
::warning
**On a self-hosted portal this limiter is normally inert.** It works from the `operating` / `operating_reset_at` counters in the response's `time` block, and the portal sends them only when its own operating limiter is active — on-premise that reads the `rest` module option `load_limiter_active`, whose default is `N` and which nothing in the product ever sets. Without the counters the SDK skips the bookkeeping rather than invent a `0`, so `OperatingLimiter` neither delays nor reports anything. `RateLimiter` and `AdaptiveDelayer` are unaffected. If you need operating-time throttling on-premise, the administrator has to enable the option **and** configure `load_limiter_connection_name`. We have no verified recipe for that yet — see the note below the table.
::

A self-hosted portal is therefore in one of three states, and the middle one is the trap. **Only the first row is measured**; the other two are read from the `rest` module's PHP sources and have not been produced on a live portal, because switching the option on carries a 300-second REST ban:

| Portal state | What arrives in `time` | Usable by the limiter |
|---|---|---|
| `load_limiter_active` unset (default `N`) | no counters at all | No — and honestly so: the SDK skips the bookkeeping |
| `load_limiter_active = Y`, no `load_limiter_connection_name` | `operating: 0`, always | **No — the number is a lie** |
| `load_limiter_active = Y` + a configured connection | real values | Yes |

The middle state is worse than the first, because the client sees the field and believes it. On the wire it is indistinguishable from a cloud portal that reports `operating: 0` because nothing has been consumed yet.

Be precise about where that costs you, though: in **delay** the two states behave identically — a stored `0` yields a zero wait from `getTimeToFree()` and a zero usage percentage in `AdaptiveDelayer`, exactly as no statistics at all would. The difference shows in what you are told: in the middle state `getStats()` reports the method with a confident `0.00`, whereas with no counters the method does not appear in `operatingStats` at all. The lie is in the readout, not in the throttling. This is also why the SDK never synthesises a `0` when the counters are missing: a fabricated zero would put every portal into that middle state.

Every cloud portal we have measured sends the counters, on `restApi:v2` and `restApi:v3` alike.

::caution
**Open question, and it changes what the rows above mean.** `LoadLimiter` gates whether the counters are *sent*. Whether the same gate also governs whether the portal *enforces* the operating limit is read from source, not measured. If it does, a default self-hosted portal is not throttling by operating time at all — and an SDK limiter that stays quiet is matching the portal, not missing anything. If it does not, the portal throttles without telling you. Until that is measured on a disposable box, read this section as "here is what the counters do", not as a list of deficiencies.
::
