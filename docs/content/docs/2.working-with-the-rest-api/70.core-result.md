...
`getErrorsByKey()` / `getErrorMessagesByKey()` return a `Record`{lang="ts-type"} **snapshot** keyed by that identifier, so you can tell which call failed — unlike `getErrors()` / `getErrorMessages()`, which drop the keys, and unlike the live `errors` `Map`{lang="ts-type"} getter. Keys are meaningful only for **object / named-command** batches (keyed by the command label). An **array-mode** batch does not produce stable per-command keys — a per-command failure gets a generated UUID, an envelope-level soft error the internal `base-error` key — so prefer `getErrors()` / `getErrorMessages()` there. (`addErrors()` also uses generated UUIDs.)
