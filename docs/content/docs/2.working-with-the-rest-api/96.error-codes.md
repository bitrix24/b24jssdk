...
| `JSSDK_PULL_SEND_REFUSED` | `PullClient.sendMessage()` / `sendMessageToChannels()` handed the frame to the transport and it would not take it — the WebSocket is not open, or long-polling has no publication path. The connector log carries the specific reason. Note that the absence of this error is not a delivery receipt: it means the transport accepted the frame, and long-polling does not await the response. When an accepted frame then goes nowhere, the reason is usually not an error at all but a WebSocket **close code** — the push server refuses a frame by closing the connection rather than by answering. `CloseReasons`{lang="ts-type"} names them: `WRONG_REQUEST_DATA`{lang="ts-type"} (4013), `NO_CHANNELS_FOUND`{lang="ts-type"} (4017), `PRIVATE_CHANNEL_NOT_ALLOWED`{lang="ts-type"} (4020), `INVALID_CHANNEL_SIGNATURE`{lang="ts-type"} (4021) and others. They never reach a caller as an error, only on the socket's `close` event. Before 3.0.0 the refusal was returned as a `false` the caller had to notice. |
