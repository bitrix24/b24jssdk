...
* **filter:** new `FilterV3` typed builder for the REST API v3 filter grammar (array-of-triples with AND/OR/NOT groups, §3). Exposes `eq`/`ne`/`gt`/`ge`/`lt`/`le`/`in`/`between` leaf helpers, `and`/`or`/`not` combinators, and `build(...)` to assemble the top-level (AND-joined) `params.filter` array — skipping falsy nodes so conditionals inline cleanly. Validates the operator set and `in`/`between` shapes on the client (a bad operator fails fast instead of as a server `UNKNOWNFILTEROPERATOREXCEPTION`). Verified live against a v3 portal. `import { FilterV3 } from '@bitrix24/b24jssdk'`
