...
describe('skills-live @skills', () => {
  ...
  describe('b24jssdk-rest/SKILL.md', () => {
    ...
    it('actions.v3.aggregate.make — @experimental, asks the portal which methods publish it', async () => {
      ...
      // future module will.
      //
      // So the open question is availability, and this asks the portal for it in
      // **one call** instead of six guesses. `rest.documentation.openapi` lists
      // every path the portal publishes, so this covers the whole portal rather
      // than the six modules someone thought to name — and it cannot print
      // anyone's row counts, which is why the `::warning` block this case used
      // to need is gone from VERIFICATION.md.
      //
      // The predecessor called `*.aggregate` on six modules with
      // `select: { count: ['id'] }`. Every line came back `SOFT` on every portal
      ...
      console.log(
        ...
        + `  portal publishes ${all.length} v3 method(s)\n`
        + `  *.aggregate: ${aggregates.length === 0 ? '(none)' : aggregates.join(', ')}\n`
        + '  Read: none = no shipped module opts into AggregateOrmActionTrait on this\n'
        + '  portal, so `AggregateV3` stays @experimental and the docs keep telling\n'
        + '  readers to reduce a callList client-side. Any name = the exit condition\n'
        + '  is met; report it, and the tag can be revisited against a real method.'
      )
