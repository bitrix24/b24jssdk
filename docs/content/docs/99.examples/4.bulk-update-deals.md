...
- `ParamsFactory.getBatchProcessing()` makes the client more cautious, not more permissive: it halves the request rate (`drainRate` 2 → 1, `burstLimit` 50 → 30) and **lowers** the thresholds at which it warns and starts adding delay (both 80% → 50%), so a long migration backs off earlier and keeps the portal queue responsive for live UI traffic. The operating budget itself is unchanged.
- Naming the request via `options.requestId` makes it easy to filter portal-side logs. It does **not** deduplicate an accidental re-run — it is a tracking tag, and the portal happily writes twice. On `restApi:v3` an [`idempotencyKey`](/docs/working-with-the-rest-api/call-rest-api-ver3/#idempotency-key) is what prevents that.
