...
The default 30-second axios timeout is too short for heavy operations like
`crm.documentgenerator.document.add` (document rendering) or `crm.deal.add`
with large payloads. On a client-side timeout the SDK retries by default,
which can create **duplicate entities** on the server.
...
for the recommended configuration (raise the axios timeout + set
