...
* **pull:** `PullClient.getDebugInfo()` no longer exposes the push JWT (`token`) or the private `CHANNEL_ID`s in its `Path` field — the connection path is run through a new `redactSensitiveUrl()` (which reuses the core redactor's key set plus the `CHANNEL_ID` key) before it is surfaced in this developer-facing debug dump. The live connector path is unchanged; only the debug view is masked (#148)
