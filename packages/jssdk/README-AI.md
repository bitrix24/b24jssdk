...
- `initializeB24Frame(options?: { restrictionParams?, httpOptions? }): Promise<B24Frame>` — `httpOptions` is axios config (`adapter` above all: a browser gets `fetch`, `{ adapter: 'xhr' }` goes back); the first call's options win, a later call returns the frame already built
