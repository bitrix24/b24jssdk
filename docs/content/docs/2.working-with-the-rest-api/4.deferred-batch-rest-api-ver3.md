...
2. The portal runs them in the background. The job's status goes from
   `pending` to `done`, or to `error`. The portal also defines `processing`;
   the measured jobs went straight from `pending` to `done`.
...
on each, a deferred batch is one request to start, then status checks until
it is done.

::warning
Deferred batches depend on the portal's **plan**. On a plan without them every
call answers `FEATURE_NOT_AVAILABLE_ON_CURRENT_PLAN`.
::

::note
**Measured on a webhook, from Node.js.** Using it from a browser (`B24Frame`)
...
| `idempotencyKey` | `string`{lang="ts-type"} | — | Sent with `add`. Use one key per run: a retry of an interrupted `make()` with the same key then does not start a second job. A key reused after a finished `make()` deleted its job replays the id of a job that no longer exists. See [Idempotency-Key](/docs/working-with-the-rest-api/call-rest-api-ver3/#idempotency-key). |
...
a `list`, and so on. The row carries no `time` block. How a failed command
appears in the rows is not yet measured, so check each row rather than assume
it holds a result.
