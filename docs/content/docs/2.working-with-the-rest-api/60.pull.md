...
::caution
`PullClient` also exposes `sendMessage()`. **It cannot work in an application**:
it resolves the recipients' channels through `pull.channel.public.list`, which is
not part of the application REST surface.

Until 3.0.0 that failure was invisible — the call resolved as if the message had
been accepted, and the push server dropped it without a word. It now rejects with
`JSSDK_PULL_PUBLIC_IDS_UNAVAILABLE`. Use `pull.application.event.add`, as above.

`sendMessageToChannels()` is a different case and is **not** affected: the caller
supplies the channel ids, so no lookup happens and that code cannot be raised.
::
