 ...
 * still does, so that contract is untouched. It stays out of `TypeHttpOptions`
 * because the SDK's own `Content-Type` and `Authorization` are decided per
 * request, where an instance header cannot reach them — see `abstract-http.ts`.
 *
