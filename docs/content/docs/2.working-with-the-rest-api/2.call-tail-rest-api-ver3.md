...
Ask the portal rather than trusting that sample: `rest.documentation.openapi` returns every v3 method it publishes, so grep its paths for `.tail`. See [Discovering v3 methods](/docs/working-with-the-rest-api/discovering-v3-methods/).
...
- `.getData(): T[]`{lang="ts-type"} — the rows.
...
The walk asks for rows strictly past the last value it saw, so the field has to carry the whole ordering by itself. Three requirements, and only the last one produces an error if you get it wrong:

- **Unique.** Rows sharing the last row's value on a page boundary are skipped, silently — you get a plausible result that is quietly short. `id` is the safe default for a reason.
- **Scalar.** An object- or array-valued field is read as a fresh reference on every page, so the stall guard — which compares values — never fires, and the walk runs until `maxPages` stops it. Such fields are ordinary in a v3 response.
- **Readable.** The value has to come back or the walk cannot advance; the SDK appends the field to an explicit `select` for you.

For a non-numeric field, pass `initialValue` as well: the default is the number `0`.
...
> `callTail.make: the cursor field "id" must not appear in `filter` — the server orders and pages by it and will reject a filter on the same field (INVALIDFILTEREXCEPTION). Remove it from `filter`.`
...
**Two of these come back, four reject.** Only the last two are bounds you set, so the rows already collected are correct and are handed back with the error attached. The other four — a refused filter, a missing `initialValue`, a stalled or backwards cursor — reject the promise, because there is no result worth keeping. So `isSuccess` alone is not enough:
...
try {
  ...
  // Filter refused, DESC without initialValue, or the cursor stopped advancing.
  console.error(error)
}
```
