...
| `JSSDK_FILTER_V3_INVALID_FIELD` / `JSSDK_FILTER_V3_INVALID_OPERATOR` / `JSSDK_FILTER_V3_INVALID_IN` / `JSSDK_FILTER_V3_INVALID_NODE` | 400 | `FilterV3` builder | Client-side validation while building a v3 filter: empty field name, operator outside the 8 allowed, `in` value not a non-empty array, or a malformed node passed to `build()` (e.g. a forgotten spread). |
