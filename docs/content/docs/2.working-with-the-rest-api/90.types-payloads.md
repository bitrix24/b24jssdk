...
`duration` and `processing` are **not usable as timings**: the portal truncates the underlying timestamps to whole seconds, which inflates `duration` by up to a second and leaves `processing` reading `0` for almost every call. The SDK measures its own durations and reads neither.
