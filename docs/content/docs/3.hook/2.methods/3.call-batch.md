...
description: 'Use B24Hook.callBatch() to execute batch requests to the Bitrix24 REST API. This allows you to execute up to 50 different commands in a single HTTP request, significantly increasing performance for bulk data operations.'
navigation:
  title: callBatch
links:
  - label: B24Hook
...
| **`isHaltOnError`** | `boolean`{lang="ts-type"} | No | **Halt on error.** If `true`, then execution of the batch is halted if an error occurs in any of the commands. The default is `false` - commands are executed even if some fail. |
| **`returnAjaxResult`** | `boolean`{lang="ts-type"} | No | **Return value format.** If `true`, the method returns a `Result` object containing either a `Record<string \| number, AjaxResult>` or an `AjaxResult[]` (depending on the `calls` format). If `false` (default), a standard Bitrix24 batch response is returned. |
