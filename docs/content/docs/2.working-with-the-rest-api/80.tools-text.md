...
- **Use `toB24Format` for every date you send to the REST API.** Passing a raw `Date` or ISO string with the wrong offset is the most common cause of filter mismatches — see [Filtering](/docs/working-with-the-rest-api/filtering/) for date-range filter examples.
