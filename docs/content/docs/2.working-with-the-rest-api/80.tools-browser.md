...
links:
  ...
  - label: scroll-size
    iconName: GitHubIcon
    to: https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/tools/scroll-size.ts
---
...
Alongside `Browser`, `src/tools/` ships a tiny `scroll-size` helper that reports the scrollable dimensions of the current document — the `max` of `scrollWidth`/`offsetWidth` and `scrollHeight`/`offsetHeight` of `document.documentElement`:

```ts-type
// backing util — packages/jssdk/src/tools/scroll-size.ts
useScrollSize(): { scrollWidth: number, scrollHeight: number }
```

::note
The helper is a `default` export of `tools/scroll-size.ts`, so it is **not** re-exported as a named symbol from `@bitrix24/b24jssdk` — the barrel's `export *` does not forward default exports. Read these values through the frame API instead. It reads `document.documentElement`, so it only works in a browser context.
::

Inside an embedded application, [`B24Frame.parent.getScrollSize()`](/docs/working-with-the-rest-api/frame-parent/) exposes exactly this util:

```ts
// $b24 is a B24Frame instance
const { scrollWidth, scrollHeight } = $b24.parent.getScrollSize()
