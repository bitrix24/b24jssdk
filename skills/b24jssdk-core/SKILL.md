...
`setRestrictionManagerParams` **replaces the parameters you name and keeps the rest** — `setRestrictionManagerParams({ maxRetries: 5 })` after the block above leaves `hardErrorCodes` and the others intact. The merge is **shallow**: `rateLimit` / `operatingLimit` / `adaptiveConfig` are replaced whole, and their types have no optional fields, so supplying one means supplying all of it. Spreading `...ParamsFactory.getDefault()` first, as above, is still right when you mean to start from the defaults rather than from the current state. (Before v2.3.0 the method replaced the whole configuration and a partial update reset everything it did not mention — #479.)
