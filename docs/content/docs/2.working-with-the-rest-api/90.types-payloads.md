...
The two `operating*` counters are **optional**, and their absence is the normal state on a self-hosted portal rather than an edge case: the portal adds them only while its own limiter is active, which on-premise reads the `rest` module option `load_limiter_active` — default `N`, and nothing in the product ever sets it. The same switch gates enforcement, so a portal that sends no counters is not limiting either. The SDK skips its operating-time bookkeeping in that case and does not invent a `0`, which would be indistinguishable from a real "nothing consumed yet".

Two further things measured on a self-hosted build, both of which a reasonable reading of these fields gets wrong:

- **`operating_reset_at` can be absent while `operating` is present**, and when it *is* present it means either the stored buckets' expiry or simply "start of this request plus the window" — the response cannot tell you which;
- **`operating` being present does not mean it accumulates.** On a portal whose limiter is switched on but has no storage configured it arrives on every response and never grows.

See [Limiters](/docs/working-with-the-rest-api/limiters/#what-the-counters-mean-once-they-arrive) for the three states and how a box is configured out of the middle one.

`duration` and `processing` are **not usable as timings** on the build this was measured on (`main 26.700.0`): the portal truncates the underlying timestamps to whole seconds, which inflates `duration` by up to a second and leaves `processing` reading `0` for almost every call. The SDK measures its own durations and reads neither.
