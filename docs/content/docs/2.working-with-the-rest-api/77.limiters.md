...
By default the SDK retries failed requests up to `maxRetries` times. For
**non-idempotent** REST methods (anything that creates an entity, e.g.
`crm.documentgenerator.document.add`, `crm.deal.add`, file uploads) this is
unsafe: a heavy request can complete server-side **after** the client has
already given up with a `REQUEST_TIMEOUT`. Retrying then creates duplicate
entities.

For these calls:

1. **Raise the axios timeout** so the client waits long enough for the response:
...
2. **Disable retries on transport errors** — when a `NETWORK_ERROR` or
   `REQUEST_TIMEOUT` is raised, the SDK throws immediately instead of retrying:
   ...
   You can also drop `maxRetries` to `1` to disable retries altogether for
   any error.
