...
**`sendMessage()` cannot work in an application**: it resolves the recipients'
channels through `pull.channel.public.list`, which is not part of the
application REST surface. It rejects with
`JSSDK_PULL_PUBLIC_IDS_UNAVAILABLE`{lang="ts-type"}. Use
`pull.application.event.add`, as above.
