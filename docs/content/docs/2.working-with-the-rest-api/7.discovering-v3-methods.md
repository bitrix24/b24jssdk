...
[`Aggregate.make()`](/docs/working-with-the-rest-api/choosing-the-right-method/)
...
* **For one entity's fields**: [`<entity>.field.list`](/docs/working-with-the-rest-api/discovering-entity-fields/)
  is the cheaper half of the same question. This document tells you *which methods exist*;
  that one tells you *which fields this entity has* — and carries `filterable`, `editable`
  and `requiredGroups`, which are not in the request schema here at all.
...
* **For calling a discovered method**: use [`Call`](/docs/working-with-the-rest-api/call-rest-api-ver3/),
  ...
  native [`CallTail`](/docs/working-with-the-rest-api/call-tail-rest-api-ver3/) /
  [`FetchTail`](/docs/working-with-the-rest-api/fetch-tail-rest-api-ver3/)
  when the method publishes a `tail` action.
* **Which methods page by a native cursor**: filter the same `paths` for the
  suffix — `Object.keys(doc.paths).filter(path => path.endsWith('.tail'))`. Few
  publish one, so this is worth checking before reaching for
