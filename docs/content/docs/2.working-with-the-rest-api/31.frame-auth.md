...
::tip
You rarely need to call this yourself. The SDK refreshes before a request and again on a `401`. If your app reads the token with `getAuthData()` and uses it outside the SDK, prefer [`keepAuthFresh`](/docs/working-with-the-rest-api/frame-initialize-b24-frame/#keeping-the-token-alive) over your own timer.
::
