...
Every mutator returns the result itself, parameterised with the same `T`{lang="ts-type"} — so a chain keeps the payload type. Until v2.2.0 `addError` / `addErrors` were declared as returning a bare `IResult`{lang="ts-type"} (that is, `IResult<any>`{lang="ts-type"}), which silently degraded `getData()` to `any`{lang="ts-type"} for anything read after a chained call.
