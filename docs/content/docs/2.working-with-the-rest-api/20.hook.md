...
| `options.httpOptions` | `TypeHttpOptions`{lang="ts-type"} | No | Axios settings for both transports, merged over the SDK's own defaults — `adapter` above all. Keys outside `TypeHttpOptions`{lang="ts-type"} are dropped at construction and their names logged. See [Configuring the axios instance](/docs/working-with-the-rest-api/core-http/#configuring-the-axios-instance). |
...
Disables the runtime warning emitted by both HTTP clients when `B24Hook`{lang="ts-type"} is detected in a browser-like runtime — the main thread or a Web, Shared or Service Worker, where the bundle is equally readable. Use only when you intentionally proxy the webhook through a server-side wrapper that strips the secret before any browser request.
