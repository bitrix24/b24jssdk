...
- When handling `ONAPPINSTALL` / `ONAPPUNINSTALL` callbacks, verify the `application_token` before acting — see [Security patterns for event receivers](/docs/working-with-the-rest-api/security/).
::
...
`options.restrictionParams` tunes the rate-limit / operating-limit / adaptive-delay configuration — see [Limiters](/docs/working-with-the-rest-api/limiters/). `options.httpOptions` carries axios settings for both transports, merged over the SDK's own defaults — `adapter` above all; see [Configuring the axios instance](/docs/working-with-the-rest-api/core-http/#configuring-the-axios-instance). Keys outside `TypeHttpOptions`{lang="ts-type"} are dropped at construction and their names logged.
...
The default refresh path sends a `POST /oauth/token/` request to `serverEndpoint` (the portal's OAuth server) with `grant_type=refresh_token`, the supplied `refreshToken`, `clientId`, and `clientSecret` in an `application/x-www-form-urlencoded` body — the credentials are never placed in the URL query string. On success the manager updates `accessToken`, `refreshToken`, `expires`, `expiresIn`, `clientEndpoint`, `serverEndpoint`, `scope`, and `status`.
...
Disables the runtime warning emitted when `B24OAuth`{lang="ts-type"} is detected in a browser-like runtime — the main thread or a Web, Shared or Service Worker. Use only when you proxy the OAuth tokens through a server-side wrapper.
...
A failed refresh raises `RefreshTokenError`{lang="ts-type"} when the portal answered with an OAuth error payload, and rethrows any other `Error`{lang="ts-type"} unchanged. Only a thrown value that is **not** an `Error`{lang="ts-type"} at all reaches the last resort — `new Error('Strange error: …')`{lang="ts-type"}, which carries that raw value as [`cause`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause) so it is not lost.
