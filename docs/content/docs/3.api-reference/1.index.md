...
| [`isBrowserLikeRuntime`](https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/tools/environment.ts) | Does the browser's rulebook apply here — CORS, forbidden headers, a public bundle? True in a worker as well as a browser. | [guide](/docs/working-with-the-rest-api/tools-environment/) |
...
`AppFrame` `CatalogProductImageType` `CatalogProductType` `CatalogRoundingRuleType` `CloseReasons` `EnumBitrix24Edition` `EnumBizprocBaseType` `EnumBizprocDocumentType` `EnumCrmEntityType` `EnumCrmEntityTypeShort` `ListRpcError` `LsKeys` `MessageCommands` `MessageManager` `ProductRowDiscountTypeId` `RpcMethod` `SenderType` `ServerMode` `SystemCommands` `convertBizprocDocumentTypeToCrmEntityTypeId` `getDocumentId` `getDocumentType` `getDocumentTypeForFilter` `getEnumCrmEntityTypeShort` `versionManager`
