...
[`idempotencyKey`](/docs/working-with-the-rest-api/call-rest-api-ver3/#idempotency-key)
...
- **Treat `member_id` as untrusted too.** It arrives on the same unauthenticated endpoint and then becomes a key in your datastore and a value in your logs. Check its shape before either — a newline in it forges a log line that reads like a real install for another portal, and nothing bounds its length. Separately, do not key an in-memory store on a plain object literal: `__proto__` is a setter, so the write is silently dropped, and `constructor` / `toString` are found as inherited functions on a lookup that should have returned nothing. A `Map` or `Object.create(null)` has that property; a record type does not.
