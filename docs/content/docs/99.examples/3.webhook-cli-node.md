...
A copy-pasted-with-typo webhook URL fails with a 401 wrapped in `SdkError` and the script exits with code `1`. A network hiccup retries up to three times before surfacing the failure — see [Restrictions System](/docs/working-with-the-rest-api/limiters/).
