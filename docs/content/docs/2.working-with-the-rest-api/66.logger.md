...
**TelegramHandler**
[Sends logs](/docs/working-with-the-rest-api/logger-telegram/) to Telegram chat. In a browser-like runtime — the main thread or any Web, Shared or Service Worker — it sends nothing and warns in the console instead, because the request would carry the bot token. Server-side, it sends a message via the Telegram Bot API.
