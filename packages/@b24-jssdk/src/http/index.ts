...
import { AjaxResult, type AjaxQuery, type AjaxResultParams } from "./ajaxResult";
import {AjaxError} from "./ajaxError";
import { LoggerBrowser } from '../logger/browser';
import type { AuthActions, AuthData } from '../types/auth';
import type { AuthError, BatchPayload } from '../types/payloads';
...
export default class Http
{
	...
	)
	{
		...
		this.#clientAxios = axios.create({
			baseURL: appFrame.getTargetOriginWithPath(),
			headers: {},
			...options ?? {}
		});
	...
	{
		let authData = this.#authManager.getAuthData();
		if(authData === false)
		{
			authData = await this.#authManager.refreshAuth();
		}
