...
`setData` is chainable. The mutators return the polymorphic `this`{lang="ts-type"} rather than `Result<T>`{lang="ts-type"}, so chaining off a subclass keeps the subclass: `ajaxResult.addError(e).getStatus()`{lang="ts"} still resolves. Before v2.2.0 they were declared as `Result<T>`{lang="ts-type"} and the chain widened away everything `AjaxResult`{lang="ts-type"} adds. Note: `AjaxResult`{lang="ts-type"} **disables** `setData` and throws `ReferenceError`{lang="ts-type"} on call.
...
`getErrorsByKey()` / `getErrorMessagesByKey()` return a `Record`{lang="ts-type"} **snapshot** keyed by that identifier, so you can tell which call failed — unlike `getErrors()` / `getErrorMessages()`, which drop the keys, and unlike the live `errors` `Map`{lang="ts-type"} getter. For batches the key tells you which command failed: an **object / named-command** batch keys by the command label, and an **array-mode** batch keys by the command's **numeric position** (`'0'`, `'1'`, … — since v1.4.0 / [#255](https://github.com/bitrix24/b24jssdk/issues/255); previously these were random UUIDs). An envelope-level soft error not tied to one command uses the internal `base-error` key, and `addErrors()` (no explicit key) still uses generated UUIDs — for those, prefer `getErrors()` / `getErrorMessages()`.
