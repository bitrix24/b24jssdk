...
2. **Was it read correctly?** `options['place']` is `unknown`{lang="ts-type"}, so
   narrow it before use — and remember the key case is the portal's, not a
   convention. See [placement options](/docs/working-with-the-rest-api/frame-placement/#options).
