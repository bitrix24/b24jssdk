...
`JSSDK_ACTION_CURSOR_WENT_BACKWARDS`{lang="ts-type"} on the second page instead
of after ten thousand. What remains for the ceiling is a walk that is genuinely
larger than expected, or one whose cursor values the SDK cannot order. See
[Behaviour: a cycling or backwards cursor now stops the walk](/docs/getting-started/migration/v3/).
::
