...
links:
  ...
  - label: TypeHttpOptions (allowlist)
    iconName: GitHubIcon
    to: https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/core/http/http-options.ts
  - label: HttpV2
    iconName: GitHubIcon
...
[BatchV3](/docs/working-with-the-rest-api/batch-rest-api-ver3/) and
[BatchV2](/docs/working-with-the-rest-api/batch-rest-api-ver2/) pages — the two
...
- **`User-Agent`**, on server-side runtimes only — a browser **and a worker** forbid the header, so it is not set in either.
...
::warning
**`maxRedirects` is forced to `0` only where a credential rides outside the
body.** A `restApi:v3` batch, or a `restApi:v3` call with `idempotencyKey`, on a
non-hook transport outside a browser carries the access token in an
`Authorization: Bearer` header, and a redirect would take it along (the
`actions.v3.deferredBatch` result-file download refuses redirects too, since its
...
By default the constraint is per-request, not on the instance: nothing else the
...
is unaffected. Same narrow scope: a `restApi:v3` batch, or a `restApi:v3` call
with `idempotencyKey`, on a non-hook transport.
...
`B24Hook`{lang="ts-type"} and `B24OAuth`{lang="ts-type"} enable a runtime warning when used in a browser-like runtime — the main thread or any worker (their secrets must never reach the client, and a worker's bundle is as public as the page's). Toggle it from the parent class via `b24.offClientSideWarning()`; this method is the underlying primitive.
