...
::note{title="📝 NOTE: The `B24Hook.callListMethod()` Method is REPLACED"}
...
`Promise<Result<T[]>>`{lang="ts-type"} — a promise that resolves to a [`Result`](/docs/core/result/) object containing the complete array of items of type `T`. This object provides:

* `.getData(): T[]`{lang="ts-type"} — returns the complete array of all retrieved data.
* `.isSuccess: boolean`{lang="ts-type"} — a flag indicating the successful execution of all requests.
