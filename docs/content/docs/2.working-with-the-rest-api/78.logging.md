...
`/rest/<userId>/<secret>/` — wherever such a URL appears in a string. That secret
is neither a key nor a `key=value` pair, so neither pass above can see it.

It turns up in a log because the redactor runs over **response bodies** as well
as over the params you sent, and a portal method can *return* such a URL:
`rest.deferredbatch.downloadresult` answers with a `downloadUrl` built from the
calling webhook.
...
- **Request headers — including one the SDK sets itself.** On a server-side
  ...
  you configure through your own axios options sits beside it. In a browser the
  same batch carries it in the request **URL** instead (`?auth=<token>`), because
  the portal's CORS policy does not allow the header — so a logger of your own
  that records request URLs will capture it. The SDK does not log the URL. The redactor
