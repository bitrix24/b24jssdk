...
::note
**Values the SDK cannot order are not called backwards.** A cursor whose type
changed (`100`{lang="ts-type"} then `'100'`{lang="ts-type"}), or string ids of different lengths (`'9'`{lang="ts-type"} then
`'10'`{lang="ts-type"}, where lexicographic order disagrees with numeric), fall back to the
equality check they had before. The guard declines to judge rather than guessing
— a false "backwards" would reject a walk that is working.
::
