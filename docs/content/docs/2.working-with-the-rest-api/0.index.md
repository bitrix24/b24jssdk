...
A second batch warning names a key on a **command** whose arguments are then read
by nobody — `query` above all, the portal's own wire spelling on `restApi:v3`,
where the SDK's key is `params`. That one is invisible without it: the command
