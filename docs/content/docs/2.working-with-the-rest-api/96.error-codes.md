...
::caution
**A close code is evidence when it arrives; its absence proves nothing.** Only a
structurally unparseable frame produces `4013`{lang="ts-type"}. A message whose
fields are well-formed but wrong is accepted, broadcast, and simply does not do
what you meant — no close, no error, no sign.
::
