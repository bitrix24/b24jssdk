...
* **logging:** a credential-handling audit of the surfaces **outside** the core HTTP layer closes the debug-log leaks the #151/#229 redactor never reached. The pull client carried the most: the JSON-RPC layer logged unknown / error server frames verbatim (an opaque frame can carry a channel `signature` HMAC), and `PullClient`'s `logMessage` / broadcast-error contexts logged raw `{ command, params, extra }` — both now run through `redactSensitiveParams`; the private / shared `CHANNEL_ID`s are masked in the connect logs and the raw push frame is truncated. The Frame transport leaked auth tokens two ways: the `init` log dumped the full handshake (`AUTH_ID` / `REFRESH_ID` / `MEMBER_ID`) and is now a safe projection (`PLACEMENT` / `LANG` / `INSTALL` / `IS_ADMIN` / `FIRST_RUN`), and the inbound `postMessage` handler logged `event.data` (which carries the refreshed token) and now logs only the callback id / origin. `B24Hook` parse / format errors no longer echo the webhook URL — the secret sits in its path (`/rest/<userId>/<secret>/`). `signature` joins the redactor's credential-key set. The shared-origin `localStorage` pull-config cache is documented as accepted residual risk (a sibling app on the same portal origin can already read it; the SDK does not own that boundary). Regression specs pin each surface; no behaviour change for callers (#43)
