...
* **core:** the `AjaxResult` paging members are **no longer scheduled for removal**. `2.1.0` announced `isMore()`, `hasMore()`, `getTotal()`, `getNext()` and `fetchNext()` as going away in `3.0.0`; that plan is withdrawn and none of the five is `@deprecated` any more. **If you already migrated to `actions.v{2,3}.{callList,fetchList}`, nothing breaks and staying there is still the better choice** — those helpers hide the offset bookkeeping and are the only ones that work under `restApi:v3`. But you are no longer required to move, and for a row count under `restApi:v2` you should not, because nothing else answers it. The removal set was assembled on the criterion "reads a `restApi:v2` envelope field", which describes the protocol rather than the user: `restApi:v2` is where most of the Bitrix24 REST surface still lives and is not going away soon, so deleting methods that work there today would break running code with no replacement on offer. `getTotal()` is the sharpest case — `SuccessPayload` omits `total` by design, the list helpers iterate without exposing it, and the `aggregate` action (`count` / `countDistinct`) is `restApi:v3`-only *and* `@experimental`, unverified against a live portal; removing it would have left a v2 consumer counting rows by downloading all of them. All five are documented as **`restApi:v2`-only**, which they always were: under `restApi:v3` `getTotal()` returns `0` and `isMore()` returns `false` **because the field is absent** — not because the count is zero or the pages ran out, so do not branch on either — and `getNext()` / `fetchNext()` throw `JSSDK_CORE_METHOD_NOT_SUPPORT_IN_API_V3`, deliberately, since a silent `false` would be indistinguishable from "last page". Note also that the `2.1.0` entry's "emits a runtime deprecation warning" applies to the `AbstractB24` shortcuts and `LoggerBrowser`; the `AjaxResult` methods never emitted one. `#277` continues to cover what does still go in `3.0.0`: `callMethod`, `callListMethod`, `fetchListMethod`, `callBatch`, `callBatchByChunk`, `AbstractB24.batchSize`, and `LoggerBrowser` / `LoggerType`.
