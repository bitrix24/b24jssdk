...
On failure it rejects. An `Error`{lang="ts-type"} raised by the batch call — including `AjaxError`{lang="ts-type"} and `SdkError`{lang="ts-type"} — is rethrown untouched, so you keep its `code` / `status`. Only a thrown value that is not an `Error`{lang="ts-type"} at all gets wrapped, in `new Error('Failed to load data')`{lang="ts-type"} with that raw value attached as [`cause`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause) so it is not lost.
