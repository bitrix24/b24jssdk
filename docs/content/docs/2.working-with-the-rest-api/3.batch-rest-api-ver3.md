...
* `R.ref('alias.path.to.field')` → `{ $ref: '…' }` — a single value. Only `item`
  ...
  with HTTP 400 `INVALIDSELECTEXCEPTION`, measured.

  ::note
  **What `add` and `update` return is the module's choice.** With the stock ORM
  action traits it is an id and a boolean — `AddResponse` declares one property,
  `public int $id`, and `UpdateResponse extends BooleanResponse`; measured through
  a module using them, `add` answered `{ result: { id: 22 } }`{lang="ts-type"} and
  `update` answered `{ result: true }`{lang="ts-type"}, while `get` answered
  `{ item: … }`{lang="ts-type"}. A module can declare its own instead: one measured
  on a cloud sandbox returned the whole affected object under `result.item` from
  both. Read the shape the module you are calling documents rather than assuming
  either.
  ::
