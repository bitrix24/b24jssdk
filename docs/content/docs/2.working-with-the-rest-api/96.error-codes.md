...
| `JSSDK_HTTP_REDIRECT_BLOCKED` | A redirect answered the one request the SDK refuses to follow — a `restApi:v3` batch on a non-hook transport, carrying an access token a redirect to a subdomain would take along. Raised where the refusal is otherwise invisible: on the `fetch` adapter the response is opaque (status 0, empty body) and axios resolves it. Outside a browser the same refusal arrives as a plain 301, and on a hook transport as `JSSDK_INTERACTION_BATCH_STRATEGY_V3_EMPTY_COMMAND_RESPONSE` — a webhook sends no token, so its requests never carry `maxRedirects: 0`. Deterministic, so it is not retried. Point the SDK at the final URL. |
