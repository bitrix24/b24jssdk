...
::caution
`PullClient` also exposes `sendMessage()` and `sendMessageToChannels()`. **Neither
can work in an application**: they resolve the recipients' channels through
`pull.channel.public.list`, which is not part of the application REST surface.

Until 3.x that failure was invisible — the call resolved as if the message had
been accepted, and the message was dropped by the push server without a word. It
now rejects with `JSSDK_PULL_PUBLIC_IDS_UNAVAILABLE`. Use
