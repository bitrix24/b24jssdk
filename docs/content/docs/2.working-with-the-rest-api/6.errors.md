...
| `JSSDK_AGGREGATE_V3_INVALID_FUNCTION` / `JSSDK_AGGREGATE_V3_INVALID_SELECT` | 400 | `AggregateV3` action | Client-side validation of a v3 `aggregate` select: a function outside `sum`/`avg`/`min`/`max`/`count`/`countDistinct`, a select value that is neither a `string[]` nor a `{ field: alias }` map, or a select naming **no aggregate column at all** (`{}`, `{ count: [] }`, `{ count: {} }`) — the portal answers that last case with a bare 500, so it is refused before the request goes out. The count is over the whole select: an empty list beside a non-empty one is accepted, because the portal accepts it. |
