...
Inside Bitrix24, open any placement handled by this app. The iframe loads the page, the script logs `initializeB24Frame done`, the parent window title becomes "My Bitrix24 App", and the iframe resizes to the content. Toggling the theme persists across reloads (the value lives on the portal, not in `localStorage`). Clicking "Open deals in slider" opens `/crm/deal/list/` of the same portal in a Bitrix24-styled slider.
