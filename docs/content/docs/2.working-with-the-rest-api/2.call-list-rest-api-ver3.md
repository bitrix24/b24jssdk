...
| **`limit`** | `number`{lang="ts-type"} | No | How many records to retrieve per page. Default is `50`. **A request, not a guarantee** — each method applies its own maximum, and a short page is not the end of the data. Measured: `tasks.task.list` answers 50 for `limit` 51, 100 and 1000 alike, with 60 rows available. This walker is cap-tolerant (see [Key Concepts](#performance-optimization)); hand-rolled paging on `call.make` is not. `limit: 0`{lang="ts-type"} or a non-numeric value is refused with `INVALIDPAGINATIONEXCEPTION`; a negative one answers a bare 500. |
...
`maxPages` stops after that many pages and throws `JSSDK_ACTION_MAX_PAGES_EXCEEDED`{lang="ts-type"}, naming the method. The rows already read are **not** discarded — they come back with the error attached, so check `isSuccess`. It defaults to **10 000** — a backstop rather than a policy: on `restApi:v3` that is 500 000 rows at the default page size of 50, or ten million at the maximum `limit` of 1000, and at the default drain rate of 2 requests/second roughly 83 minutes, so a walk that never ends is bounded without capping a read anyone performs. Raise it when a read genuinely needs more.
