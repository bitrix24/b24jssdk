...
TypeScript enforces this half of it for you: `RateLimitConfig` and its
siblings have no optional fields, so `{ rateLimit: { burstLimit: 7 } }` is a
compile error rather than a block that quietly inherits `drainRate` from the
previous configuration. A JavaScript caller, or one reaching for a cast, gets
the block exactly as written and nothing more.
...
// const $b24 = ...
const current = $b24.getHttpClient(ApiVersion.v3).getRestrictionManagerParams()
...
Before v2.3.0 this method **replaced the whole configuration**, so a partial
update silently reset every parameter it did not mention — including the nested
blocks. Examples in this documentation spread `...ParamsFactory.getDefault()`
first for that reason; doing so is still correct, and is what you want when you
mean to start from the defaults rather than from the current state.
