...
::note
**`ESM` is the recommended module format.** `CommonJS` is also supported — you can `require('@bitrix24/b24jssdk')` from a CommonJS project (it resolves to a first-class CJS build with external dependencies — see the note below; TypeScript files run directly with `ts-node` / `tsx` work too). A `UMD` build is shipped for `<script>` usage in the browser.
::
