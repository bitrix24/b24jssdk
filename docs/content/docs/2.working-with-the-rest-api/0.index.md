...
// v3 is symmetric, and adds the native keyset walkers for the few
// methods that publish a `*.tail` endpoint
await $b24.actions.v3.call.make(options)
await $b24.actions.v3.callTail.make(options)     // Promise<Result<T[]>>
$b24.actions.v3.fetchTail.make(options)          // AsyncGenerator<T[]>
// ...
...
- **[`DeferredBatch`](/docs/working-with-the-rest-api/deferred-batch-rest-api-ver3/)** — thousands of commands as one background job on the portal, with a status callback and decoded rows (`restApi:v3`)
...
Until #426 the option types carried an index signature, so **any** top-level key
compiled and the misplaced ones did nothing. The batch actions additionally log a
warning at runtime, for a caller the compiler never sees — plain JavaScript, or a
literal widened through a variable.
...
[BatchV2](/docs/working-with-the-rest-api/batch-rest-api-ver2/).
...
2. **Lists that fit in memory** → Use `CallList`
...
3. **Bulk operations (more than 50)** → Use `BatchByChunk`, or `DeferredBatch` (`restApi:v3`) when the portal's plan has deferred batches
...
- `requestId`{lang="ts-type"} — always specify a unique request identifier — it is a tracking tag (`bx24_request_id`), not a deduplication key; for deduplication see [`idempotencyKey`](/docs/working-with-the-rest-api/call-rest-api-ver3/#idempotency-key)
