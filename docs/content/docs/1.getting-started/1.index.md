...
::note
Using a custom logger via `setLogger(...)`? See [Logging & Credential Redaction](/docs/working-with-the-rest-api/logging/) for what the SDK redacts automatically, which patterns can still leak through a custom handler, and how to audit your log archives.
::
