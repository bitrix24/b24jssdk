...
- For a more efficient revenue roll-up, the SDK exposes `actions.v3.aggregate.make` (a single `sum` query) — usable on v3 methods that publish an `*.aggregate` action. CRM is not on v3 here, so this example aggregates client-side as shown above. Worth knowing before you switch: the summed field must be **`filterable`** on the entity (ask `<entity>.field.list` — being selectable is not enough, and on a v3 entity very few fields qualify), and a `sum` comes back as a **string** and is `null` when nothing matched, so a revenue total needs a deliberate conversion rather than arithmetic straight off the response.
