...
1. **Did the parameter arrive?** Log `$b24.placement.options` in the new frame —
   and log which frame you are in alongside it (`window.location.pathname`,
   `$b24.placement.placement`). Both the main window and the slider frame run
   your code, and in the main window `place` is legitimately absent. Without a
   frame identifier, the main window's perfectly normal "no place" line reads as
   a slider failure.
