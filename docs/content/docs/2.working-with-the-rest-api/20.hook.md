...
| `options.httpOptions` | `TypeHttpOptions`{lang="ts-type"} | No | Axios settings for both transports, merged over the SDK's own defaults — `adapter` above all. See [Configuring the axios instance](/docs/working-with-the-rest-api/core-http/#configuring-the-axios-instance). |
