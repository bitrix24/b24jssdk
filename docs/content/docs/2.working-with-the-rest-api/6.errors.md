...
| `VALIDATION_REQUESTVALIDATIONEXCEPTION` | 400 | A required request field is missing or malformed. | The response carries a `validation` array naming the field — that is what [`AjaxError.validation`](#which-field-failed-ajaxerrorvalidation) exposes. |
...
| `UNKNOWNDTOPROPERTYEXCEPTION` | 400 | A field name the entity does not have — or has, but not with the attribute this use needs. | Read the real names from `<entity>.field.list`; note the message says "unknown" even when the field exists but is not filterable. |
