...
The shape is matched narrowly on purpose: the user id is digits, the secret is
letters and digits only and at least eight of them, and it must be a whole path
segment. A REST method name carries dots (`crm.item.list`) and a short path word
(`batch`, `profile`) is under the length floor, so neither is masked — blinding
the logs would be its own defect. The host and the user id stay readable.
