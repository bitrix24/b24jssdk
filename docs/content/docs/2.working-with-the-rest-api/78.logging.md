...
The redactor also masks the **secret segment of a Bitrix24 webhook URL** —
`/rest/<userId>/<secret>/` — wherever such a URL appears in a string. That
secret is neither a key nor a `key=value` pair, so neither pass above sees it,
and a webhook secret is a bearer credential: one URL is enough to use every
scope the webhook has, with no second factor and no expiry.

This matters because the redactor runs over **response bodies** as well as over
the params you sent. A portal method can *return* such a URL —
`rest.deferredbatch.downloadresult` answers with a `downloadUrl` whose path
carries the calling webhook's own secret — so without this pass the secret
reached the `post/response` record at `info` level while the line looked
redacted, because a `?token=` sitting beside it was masked correctly.

Both shapes are covered: `/rest/<id>/<secret>/` for `restApi:v2` and
`/rest/api/<id>/<secret>/` for `restApi:v3`. The segment may also end the string
or be followed by `?` or `#`, because `.../rest/1/<secret>` with no trailing
slash is the form `B24Hook.fromWebhookUrl` accepts.

What keeps it from over-masking is the rest of the shape: the user id is digits,
the secret carries no dot and is at least eight characters, and it must be a
whole path segment. A REST method name carries dots (`crm.item.list`) and a short
path word (`batch`, `profile`) is under the length floor, so neither is masked —
blinding the logs would be its own defect. Matching is case-insensitive, and
hyphen and underscore are allowed inside the secret even though Bitrix24 issues
alphanumeric ones: the SDK controls neither the case a copied URL arrives in nor
the format the portal may issue next. The host and the user id stay readable.
...
The cap is also **total**: the helper accepts any value and coerces it, so a
payload with nothing to serialise is written as the literal `undefined` (or
`null`) instead of raising. That matters because these arguments are evaluated
eagerly — before the logger sees them, and regardless of log level — so a
`TypeError` raised here would propagate out of the *request*, not just out of the
logging. Two real responses reach that state: a v3 success body with no `result`
envelope (`rest.documentation.openapi` answers with a bare OpenAPI document), and
an axios error with no `response` at all (network failure, timeout, CORS) (#338).
...
- **A credential in the path of a non-Bitrix24 URL** — the path pass matches
  Bitrix24's own webhook shape. "A credential somewhere in a path" is not a
  decidable question, so a third-party URL that carries one is not covered.
- **The response body itself, beyond credentials** — it *is* redacted, but
  redaction is not omission. A portal answer is user data, and it is written to
  `post/response` at `info`. If your sink must not hold it, filter at the sink.
...
3. **Custom or deeply-nested secrets in `post/response`.** The
   `post/response` channel now runs `response.data.result` through the
   same redactor, so canonical credential keys at depth ≤ 2 are masked.
   But a secret under a **custom** key, or nested deeper than two levels,
