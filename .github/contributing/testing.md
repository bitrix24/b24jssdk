...
`test/tsconfig.json` extends [`tsconfig.base.json`](../../tsconfig.base.json) at
the repository root, which is the one place the language level, module
resolution and the full strict-flag set are written down. Until #396 it extended
`packages/jssdk/tsconfig.json` instead — the package *build* config — so an edit
there made for build reasons (`paths`, `moduleResolution`, `lib`, `target`,
whether declarations are emitted) silently changed what this gate checks, and
the person making it had no reason to look in `test/`. Four configs extend the
base today: the package, `test/`, `playgrounds/cli` and the recipes. The four
`.*-typecheck/tsconfig.json` files are generated by the block-check scripts and
are `strict: false` on purpose; `docs/` and `packages/jssdk-nuxt` extend a
Nuxt-generated config. Neither group belongs under the base.
