...
3. **Application-layer idempotency.** For non-idempotent creates that must
   survive a retry regardless of restriction config, add your own idempotency
   guard (a dedup key / unique business field checked before insert) so a
   duplicate call is a no-op even if a retry slips through.
