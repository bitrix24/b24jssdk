...
| `JSSDK_HTTP_REDIRECT_BLOCKED` | 0 | HTTP transport | A redirect answered the one request the SDK sends with `maxRedirects: 0` — a `restApi:v3` batch on a non-hook transport, which carries an access token a redirect to a subdomain would take along. Raised where the refusal is otherwise invisible: on the `fetch` adapter the answer is an opaque response (status 0, empty body) that axios *resolves*, so without this the refused request would read as an empty success. Outside a browser the same refusal arrives as a plain 301. On a **hook** transport the same opaque answer surfaces as `JSSDK_INTERACTION_BATCH_STRATEGY_V3_EMPTY_COMMAND_RESPONSE` instead — a webhook sends no token, so the request never carries `maxRedirects: 0`. Not retried: the portal will answer the next attempt the same way. Point the SDK at the final URL. |
