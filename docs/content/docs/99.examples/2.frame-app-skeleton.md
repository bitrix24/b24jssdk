...
onMounted(async () => {
  try {
    // `keepAuthFresh` is only needed if you use the token outside the SDK —
...
- The token is refreshed for you **on the request path** — an expired one is renewed before a call, and again on a `401`. This skeleton calls `app.info` through `$b24`, so it is covered. An app that instead reads `$b24.auth.getAuthData()` and sends the token to its own backend makes no `$b24` call at all, so nothing refreshes it and an idle tab loses the token: that app wants `initializeB24Frame({ keepAuthFresh: true })`, passed at the **first** init. See [Keeping the token alive](/docs/working-with-the-rest-api/frame-initialize-b24-frame/#keeping-the-token-alive).
...
- [`initializeB24Frame()`](/docs/working-with-the-rest-api/frame-initialize-b24-frame/) — including `keepAuthFresh` for apps that use the token outside the SDK.
