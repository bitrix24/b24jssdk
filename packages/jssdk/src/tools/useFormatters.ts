...
{
	private static isInternalConstructing: boolean = false
	private static instance: FormatterNumber|null = null
	private readonly locale: string
	...
	{
		if(!FormatterNumber.isInternalConstructing)
		{
			throw new TypeError('FormatterNumber is not constructable')
		}
		FormatterNumber.isInternalConstructing = false
		this.locale = locale
	}
	...
	{
		if(!FormatterNumber.instance)
		{
			FormatterNumber.isInternalConstructing = true
			FormatterNumber.instance = new FormatterNumber(locale)
		}
		return FormatterNumber.instance
	}
...
export default function useFormatter(locale: string = 'en-US')
{
	const formatterDateTime = FormatterDateTime.getInstance(locale)
	const formatterNumber = FormatterNumber.getInstance(locale)
	return {
		formatterDateTime,
		formatterNumber
	}
