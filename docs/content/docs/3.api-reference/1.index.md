...
Deferred (background) batches are reached through `$b24.actions.v3.deferredBatch`, not a separate export; their types (`DeferredBatchJob` and the rest) are listed in the [types index](/docs/working-with-the-rest-api/types-index/) and described in the [DeferredBatch guide](/docs/working-with-the-rest-api/deferred-batch-rest-api-ver3/).
...
| [`CloseReasons`](https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/types/pull.ts) | Why a connection closed — the codes the client sends, and the ones the push server sends to refuse a frame. | [guide](/docs/working-with-the-rest-api/error-codes/#pull-close-codes) |
| [`isFrameRefusalCloseCode`](https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/types/pull.ts) | Was this close code the server rejecting the frame just published, rather than the connection? | [guide](/docs/working-with-the-rest-api/error-codes/#pull-close-codes) |
...
| [`isBrowserLikeRuntime`](https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/tools/environment.ts) | Whether the browser's rules apply — true in a worker too. | [guide](/docs/working-with-the-rest-api/tools-environment/) |
