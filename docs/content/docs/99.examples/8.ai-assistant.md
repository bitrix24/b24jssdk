...
- The deal lookup is a **`restApi:v2`** call (`crm.item.get`) that happens to use camelCase fields — `crm.item.*` is camelCase for reasons of its own, not because it is v3; the v3 endpoint publishes no `crm.item.*`. The activity list uses **classic** uppercase fields (`OWNER_TYPE_ID`, `OWNER_ID`, `CREATED`). Under `restApi:v3` camelCase is the rule — see [Discovering entity fields](/docs/working-with-the-rest-api/discovering-entity-fields/).
