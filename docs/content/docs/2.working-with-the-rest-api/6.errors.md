...
| `JSSDK_ACTION_V3_LIST_FILTER_NOT_ARRAY` | 500 | `actions.v3.callList` / `fetchList` / `callTail` / `fetchTail` | `filter` had a shape the action can't use. The `restApi:v2` object dialect (`{ '>id': 100 }`) is rejected by all four: the portal reads a filter element carrying none of `type` / `logic` / `conditions` / `negative` **positionally**, so a map keyed by an operator prefix matches no condition it knows — there the operator is a position, not a prefix on the field name. A **logic group** (`FilterV3.or()` / `and()` / `not()`) is a different shape and the portal accepts one even as the whole `filter`: `callTail` / `fetchTail` pass it through, while `callList` / `fetchList` still need an array, since they append `[cursorIdKey, '>', cursor]` to it on every page and only an array can be extended. Use `[['id', '>', 100]]` or `FilterV3.build(...)`. |
