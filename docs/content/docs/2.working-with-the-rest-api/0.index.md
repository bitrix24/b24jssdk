...
A second batch warning fires once per `addCommands` call when a **command** loses
its arguments to an unread key — `query` above all, the portal's own wire
spelling on `restApi:v3`, where the SDK's key is `params`. That one is invisible without it: the command
goes out with no arguments and the portal answers HTTP 200 with the method's
defaults. See [BatchV3](/docs/working-with-the-rest-api/batch-rest-api-ver3/) and
