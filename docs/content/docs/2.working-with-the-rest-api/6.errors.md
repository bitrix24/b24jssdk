...
- `AjaxError extends SdkError` — thrown when an HTTP call to Bitrix24 fails. Adds `requestInfo` (URL, method, requestId) so you can correlate with portal-side logs.
...
try {
  ...
  if (error instanceof AjaxError) {
    ...
    console.error(error.code, error.status, error.requestInfo?.url)
  }
