...
* **core:** the API v3 supported-methods allowlist now covers the newly-published rest-v3 modules — `mail.*` (24), `humanresources.*` (22), `timeman.record.*` (3), plus new `tasks.task.*` (`result.*`, `*.field.list`/`field.get`) and `main.eventlog.field.*`. `actions.v3.*` now accepts these (it previously threw `JSSDK_CORE_METHOD_NOT_SUPPORT_IN_API_V3`); `actions.v2.*` callers of them get the existing "worth migrating to v3" warning (non-breaking, no auto-redirect). Routing is enabled but not yet portal-verified (no `// done` markers), and cross-module references seen on those pages (`user.get`, `im.*`, `disk.*`) are deferred to their own modules (#203)
