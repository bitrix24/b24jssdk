---
title: Dialog Manager Class
description: 'Used for displaying standard dialogs.'
category: 'B24Frame'
audited: 2026-05-05
navigation.title: Dialog
links:
  - label: DialogManager
    iconName: GitHubIcon
    to: https://github.com/bitrix24/b24jssdk/blob/main/packages/jssdk/src/frame/dialog.ts
---

::warning
We are still updating this page. Some data may be missing here — we will complete it shortly.
::

## Methods

### selectUser
```ts
async selectUser(): Promise<null|SelectedUser>
```

Displays a standard dialog for selecting a single user.

Shows only company employees.

Returns a `Promise` that resolves to `null` or a [`SelectedUser`](#selectusers) object.

```ts
// ... /////
$b24 = await initializeB24Frame()
// ... /////
const makeSelectUsers = async() => {
	const selectedUser = await $b24.dialog.selectUser()
	$logger.info(selectedUser)
}
```

### selectUsers
```ts
async selectUsers(): Promise<SelectedUser[]>
```

Displays a standard dialog for selecting multiple users.

Shows only company employees.

Returns a `Promise` that resolves to an array of [`SelectedUser`](#selecteduser) objects.

```ts
// ... /////
$b24 = await initializeB24Frame()
// ... /////
const makeSelectUsers = async() => {
	const selectedUsers = await $b24.dialog.selectUsers()
	
	const list = selectedUsers.map((row: SelectedUser): string => {
		return [ `[id: ${row.id}]`, row.name ].join(' ')
	})
	
	$logger.info(selectedUsers, list)
}
```

### selectAccess
```ts
async selectAccess(blockedAccessPermissions?: string[]): Promise<SelectedAccess[]>
```

Displays the standard access-rights selection dialog. The optional `blockedAccessPermissions` array hides specific permission ids (e.g. `'AU'`, `'CR'`, `'U1'`) from the picker.

Returns a `Promise` that resolves to an array of [`SelectedAccess`](#selectedaccess) objects.

```ts
// ... /////
$b24 = await initializeB24Frame()
// ... /////
const access = await $b24.dialog.selectAccess(['AU'])
$logger.info(access)
```

### selectCRM
```ts
async selectCRM(params?: SelectCRMParams): Promise<SelectedCRM>
```

Displays the standard CRM-entity selection dialog. The `params` object controls which entity types appear (`lead`, `contact`, `company`, `deal`, `quote`), whether multiple selection is allowed, and which entities are pre-selected.

Returns a `Promise` that resolves to a [`SelectedCRM`](#selectedcrm) object — a record keyed by entity type, each value an array of selected entities.

```ts
// ... /////
$b24 = await initializeB24Frame()
// ... /////
const result = await $b24.dialog.selectCRM({
  entityType: ['contact', 'company'],
  multiple: true
})
$logger.info(result.contact, result.company)
```

## Data Types

### SelectedUser

Used to represent information about a selected user in the Bitrix24 application. It contains several fields that describe the user's ID, name, photo, position, and other characteristics.

> The `sub` and `sup` fields help determine hierarchical relationships between the current user and the selected user.

- **`id: NumberString`**: User identifier. Represented as a string containing a numeric value.
- **`name: string`**: Formatted name of the user.
- **`photo: string`**: URL of the user's photo.
- **`position: string`**: User's position in the company.
- **`url: string`**: URL of the user's profile.
- **`sub: boolean`**: Flag indicating that the selected user is a subordinate of the current user. A value of `true` means the user is a subordinate.
- **`sup: boolean`**: Flag indicating that the selected user is a supervisor of the current user. A value of `true` means the user is a supervisor.

### SelectedAccess

Represents a Bitrix24 access permission picked through `selectAccess()`{lang="ts-type"}.

- **`id`**: Permission identifier — one of `AU` (all authorized users), `CR` (current user), `U<n>` (user), `IU<n>` (employee), `DR<n>` (department + subdepartments), `D<n>` (single department), `G<n>` (group), `SG<n>` (social network group).
- **`name: string`**: Human-readable label of the permission as shown to the user.

### SelectCRMParams

- **`entityType: ('lead' | 'contact' | 'company' | 'deal' | 'quote')[]`**: Which entity types to expose in the dialog.
- **`multiple: boolean`**: Allow multi-select. Default `false`.
- **`value?: { lead?: number[], contact?: number[], company?: number[], deal?: number[], quote?: number[] }`**: Pre-selected entity ids. Honoured only when `multiple: true`.

### SelectedCRM

Result shape of `selectCRM()`{lang="ts-type"}. Keyed by entity type; each value is an array of matched entities. The `id` of each entity is a tagged template literal — `L_<n>`, `C_<n>`, `CO_<n>`, `D_<n>`, `Q_<n>` — so TypeScript can narrow downstream code by entity type.
