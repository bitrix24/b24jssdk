...
The Entity API field names differ from Bitrix24 REST:

| Bitrix24 (b24jssdk) | VibeCode |
|---|---|
| `crm.deal.list` filter `'>=OPPORTUNITY': 50000` | `POST /v1/deals/search` body `{ "amount": { "$gte": 50000 } }` |
| `crm.item.list` filter `'!stageId': 'LOST'` | filter `{ "stageId": { "$ne": "LOST" } }` |
...
| Filter operators: `>=`, `<=`, `!`, `%`, `=%` | MongoDB-style: `$gte`, `$lte`, `$ne`, `$contains`, `$in` |
| Auth: webhook secret in URL or OAuth tokens | Auth: `X-Api-Key: vibe_api_…` |

Translating a working b24jssdk filter into a VibeCode body requires **rewriting** the operator syntax and field names. It's not a free swap. Picking one keeps your code consistent.

## The "AI add-on" pattern (VibeCode just for AI)

If your portal has an `vibe_api_…` key and you only want AI Router or Web Search, keep b24jssdk for Bitrix24 and call VibeCode endpoints with plain `fetch`:

```ts
import { B24Hook, EnumCrmEntityTypeId, type TypeB24 } from '@bitrix24/b24jssdk'
...
- ❌ Passing MongoDB-style filters (`$gt`, `$lt`, `$ne`, `$contains`) to `callMethod` — Bitrix24 REST does not understand them and returns 400.
- ❌ Calling `/v1/deals/search` from in-frame code and then calling `crm.deal.list` for the same deals "for consistency". Pick one source.
