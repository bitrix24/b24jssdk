...
::note
`encode` / `decode` deliberately omit the trailing `;` (e.g. `&amp` instead of `&amp;`). This matches the legacy Bitrix Framework behaviour. `decode` still accepts either form as input, plus the numeric entities (`&#38`, `&#60`, …).
::
...
- **`toKebabCase` splits on case boundaries.** `getUserId` → `get-user-id`, so it also handles `camelCase` and acronym-heavy names.
...
- **Use `toB24Format` for every date you send to the REST API.** Passing a raw `Date` or ISO string with the wrong offset is the most common cause of filter mismatches — see the [filtering guide](/docs/working-with-the-rest-api/) for date-range examples.
