...
| `JSSDK_HTTP_INVALID_IDEMPOTENCY_KEY` | `idempotencyKey` is not 1-255 printable ASCII characters. Thrown before the request leaves; the message never echoes the key. |
| `JSSDK_HTTP_REDIRECT_BLOCKED` | The portal answered with a redirect on the one request the SDK refuses to follow — a `restApi:v3` batch on a non-hook transport, which carries an access token a redirect to a subdomain would take along. Point the SDK at the final URL. |
