...
200 with the method's defaults.

Measured on `user.get` with `filter: { ACTIVE: 'N' }` against a portal whose only
...
user the filter was meant to exclude. A wrong answer, not a failure.

`query` is the *`restApi:v3`* wire spelling, which is why it is the one a caller
reaches for; on v2 it is simply a key the parser does not read.
